...
ModbusIP mb;
#else
#pragma message "MODBUS not configured! Nothing to compile. "
#endif // USE_MODBUS
...
void modbus_Setup()
{
	...
	for (int i = 0; i < nSensor; i++)
		mb.addHreg(hrDSTEMP0 + i); // Регистр температуры
	mb.addHreg(hrPSET);
	mb.addHreg(hrPNOW);
	mb.addHreg(hrANGLE);
	for (int i = 0; i < 3; i++)
...
void modbus_Update()
{
	...
	*/
	...
	if (isMBmasterOK)
	{
		...
		if (showInfo > 0)
			log_info_ln("+ MODBUS ok");
	}
	else
	{
		//	mb.Hreg(hrPSET, 0);
		if (showInfo > 0)
			log_info_ln("-- MODBUS Master OFFline ", mb.Hreg(hrSECONDS));
	}

	if (isMBmasterOK != wasMasterOK)
		setPower(0);

	mb.Hreg(hrPSET, TEH.Pset);

	for (int i = 0; i < nSensor; i++)
		mb.Hreg(hrDSTEMP0 + i, round(dsTemp[i] * 100));

//	mb.Hreg(hrPNOW, TEH.Pavg);
	mb.Hreg(hrPNOW, TEH.Pmid);
