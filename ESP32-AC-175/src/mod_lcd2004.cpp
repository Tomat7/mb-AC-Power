...
#else
#pragma message "LCD not configured! Nothing to compile. "
#endif // USE_LCD
...
void lcd_SetBackLight()
{
	...
	if (isScreenSaver)
	{
		...
		if (TEH.Pset && isBacklight && (lcdMode == SHOW_NETCONFIG))
			lcdMode = SHOW_POWER;
...
void lcd_Info()
{
	...
	{
		lcd_Power("Pnow", TEH.Pnow, 0);
		lcd_Power("Pset", TEH.Pset,  1);
		lcd_Power("Angle", TEH.Angle, 2);
		lcd_Spinner();
		lcd_Status();
	}
	else if (lcdMode == SHOW_VA)
	{
		lcd_VA();
		lcd_Spinner();
		lcd_Status();
	}
	else if (lcdMode == SHOW_NETCONFIG)
	{
		lcd_NetConfig();
	}
	else if (lcdMode == SHOW_UPTIME)
	{
		lcd_UpTime();
		lcd_LibInfo();
	}
...
void lcd_Power(const char *Sname, uint16_t Pshow, int z)
{
	check_TEH();
	float Tshow = dsTemp[z];
	LCD.setCursor(0, z);
	...
	if (z == 2)
		LCD.printf("             %6.2f%c", Tshow, deg);
	else
		LCD.printf("%s   %4dw %6.2f%c", Sname, Pshow, Tshow, deg);
...
#ifdef USE_MODBUS
	else if (isMBmasterOK)
		strStatus = "  MB_Ok";
#else
	else if (cntWiFiErrors == 0)
		strStatus = "WiFi_Ok";
#endif
	else
		strStatus = "  _OFF_";
