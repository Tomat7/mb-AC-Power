...
void encoder_Check()
{
	...
	{
		isScreenSaver = true;
		if (msTelnetPeriod == SHOW_PERIOD) msPrintPeriod = SHOW_PERIOD * 3;
		else msPrintPeriod = msTelnetPeriod;
	}
	else
	{
		isScreenSaver = false;
		if (msTelnetPeriod == SHOW_PERIOD) msPrintPeriod = SHOW_PERIOD;
		else msPrintPeriod = msTelnetPeriod;
	}
...
void encoder_ShortPress()
{
	if (isLocalMode)
	{
		...
		else log_info_ln(". ENCODER set Power: ", Pencoder);
		lcdMode = SHOW_POWER;
