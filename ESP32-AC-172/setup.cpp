...
//	log_cfg_ln(fname.substring((1 + fname.lastIndexOf('\\')), fname.lastIndexOf('.')));
//	log_cfg_ln(String(" . compiled on ") + __TIMESTAMP__);
	log_cfg_ln(fname.substring((1 + fname.lastIndexOf('\\')))); // +(" v.") + VERSION_CODE);
	log_cfg_ln(String(" . v.") + VERSION_CODE + " compiled on " + __DATE__ + " " + __TIME__);
	...
	log_cfg_ln(String(" / ") + String(ESP.getChipCores()) + " cores " + String(ESP.getCpuFreqMHz()) + " MHz");
