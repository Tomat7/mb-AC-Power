...
void setup_Web()
{
    ...
    httpd.on("/", handleRoot);
    httpd.on("/on", handleON);
    httpd.on("/off", handleOFF);
    httpd.on("/reset", handleRST);
...
void handleRoot()
{
    httpd.send(200, "text/html", SendHTML(TEH.Pset));
}
...
void handleON()
{
    ...
    httpd.send(200, "text/html", SendHTML(Pweb));
}
...
String SendHTML(uint16_t Ptmp)
{
	...
	if (mbMasterOK) snprintf(HtmlStr, 50, "<p>Modbus OK</p>\n");
